...
class DocumentRetrievalModel:
    ...
    def query(self,pQ):
        ...
        if len(sentences) == 0:
            list=[]
            list.append("Oops! Unable to find answer")
            return list
            
        # Get most relevant sentence using unigram similarity
        relevantSentences = self.getMostRelevantSentences(sentences,pQ,2)
    ...
    def getSimilarParagraph(self,queryVector):    
        ...
        return sorted(pRanking,key=lambda tup: (tup[1],tup[0]), reverse=True)[:7]
